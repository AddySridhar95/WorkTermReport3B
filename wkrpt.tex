...
\section{Results}
Since the two solutions are compared against a set of relatively subjective evaluation criteria, the AHP is the most suitable MCDA method of quantitative analysis. The alternative score for each criterion is calculated and is then multiplied with the criterion’s weighting to obtain a final score of each alternative for each criterion. Table x.x represents a set of final scores. Figure x.y is the graphical representation of the set of final scores.
...
% Figure: Graphical representation of final scores
...
This report recommends using the Atomic CSS method to style content in component based applications.
\newpage
